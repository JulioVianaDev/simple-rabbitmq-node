...
// --- Notify processors about new customer queues ---
// Uses a FANOUT exchange so ALL processor workers receive every notification.
// Each worker then checks via consistent hashing if the queue is theirs.
// A normal queue would round-robin (only 1 worker gets it = wrong worker ignores it).
var notifyExchangeAsserted = false;
var NOTIFY_EXCHANGE = config.NOTIFY_EXCHANGE;

function notifyNewQueue(queueName) {
  if (!publishChannel) return;

  function doNotify() {
    // Fanout exchange ignores routing key, but we pass the queue name for debugging
    publishChannel.publish(NOTIFY_EXCHANGE, "", Buffer.from(queueName), { persistent: false });
    console.log("  [NOTIFY] %s", queueName);
  }

  if (notifyExchangeAsserted) {
    doNotify();
    return;
  }

  publishChannel.assertExchange(NOTIFY_EXCHANGE, "fanout", { durable: false }, function (err) {
    if (err) { console.error("[!] Failed to assert notify exchange:", err.message); return; }
    notifyExchangeAsserted = true;
    doNotify();
  });
}
...
function publishToCustomerQueue(tenantId, instanceId, customerId, data, callback) {
  ...
  publishChannel.assertQueue(queueName, {
    ...
    // Notify processors: "new queue exists, start consuming it now"
    notifyNewQueue(queueName);
    doPublish();
  });
}
